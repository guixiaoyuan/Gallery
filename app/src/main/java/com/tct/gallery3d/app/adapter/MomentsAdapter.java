...
public class MomentsAdapter extends DragSelectRecyclerViewAdapter<RecyclerView.ViewHolder> implements
    ...
    public static final String TAG = MomentsAdapter.class.getSimpleName();
    ...
    public MomentsAdapter(MomentsFragment context, DragSelectRecyclerView recyclerView) {
        super(null);
    ...
    public int getSpanSize(int position) {
        if (getItemViewType(position) == VIEW_TYPE_SECTION) {
            ...
            if (mState == MomentsFragment.State.STAGGERED) {
                if (position == mSpanSizeMap.size()) {
                    position--;
                }
                int headerNum = countHeaderSize(position);
                if (position - headerNum == mSpanSizeMap.size()) {
                    return mSpanSizeMap.get(position - headerNum - 1);
                }
                return mSpanSizeMap.get(position - headerNum);
            }
        }
    ...
    public int getItemHeight(int position) {
        ...
        if (viewType == VIEW_TYPE_ITEM) {
            if (mState == MomentsFragment.State.STAGGERED) {
                int headerNum = countHeaderSize(position);
                return mItemHeightMap.get(position - headerNum);
            }
            return mRecyclerView.getWidth() / mLayoutManager.getSpanCount();
        }
    ...
    public void onClick(View view) {
        ...
        if (view instanceof PhotoItem) {
            ...
            if (mSelectionManager.inSelectionMode()) {
                ...
                data.putInt(GalleryConstant.KEY_MEDIA_ITEM_COUNT, getItemCount());
    ...
    public boolean onLongClick(View view) {
        ...
        if(!ScreenUtils.isInSplitScreen(mContext)){
            ...
            //create ClipData Object and Drag
            DragUtil.startDrag(item,view);
        }
        return true;
    }
    ...
    public void onBindViewHolderCursor(RecyclerView.ViewHolder holder, Cursor cursor) {
        ...
        switch (viewType) {
            ...
            case VIEW_TYPE_ITEM:
                ...
                boolean isStaggered = mState == MomentsFragment.State.STAGGERED;
    ...
    public void setLayoutManager(GridLayoutManager layoutManager) {
        ...
        mLayoutManager.setSpanSizeLookup(new CustomSpanSize());
    }
    ...
    private void setLayoutInfo(PhotoItem contentItem, int index, final MediaItem item) {
        ...
        mFragment.loadThumbnail(item, imageView, new ImageWorker.OnImageLoadedListener() {
            ...
            public void onImageLoaded(boolean success) {
                if (success) {
                    if (mState == MomentsFragment.State.STAGGERED) {
                        mFragment.loadGlide(item, imageView);
                    }
                }
            }
        });
        ...
        if (mState == MomentsFragment.State.MONTH) {
            contentItem.mVideoTime.setVisibility(View.GONE);
            shadowParams.height = (int) mContext.getResources().getDimension(R.dimen.moments_shadow_small_height);
        } else {
    ...
    public MomentsFragment.State getState() {
        return mState;
    }

    public List<List<ImageInformation>> getGroupList() {
        return mValuesList;
    }

    public HashMap<Integer, Integer> getSpanSizeList() {
        return mSpanSizeMap;
    }
}
