...
import com.tct.gallery3d.app.adapter.DragSelectRecyclerViewAdapter;

public class DragSelectRecyclerView extends RecyclerView implements RecyclerView.OnItemTouchListener {
    private final static String TAG = "DragSelectRecyclerView";
    ...
    public DragSelectRecyclerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private static void LOG(String message, Object... args) {
        ...
        if (!LOGGING) return;
        if (args != null) {
            android.util.Log.d("DragSelectRecyclerView", String.format(message, args));
        } else {
            android.util.Log.d("DragSelectRecyclerView", message);
        }
    }
            ...
            try {
                ...
                if (!autoScrollEnabled) {
                    ...
                    mHotspotOffsetTop = a.getDimensionPixelSize(
                            R.styleable.DragSelectRecyclerView_dsrv_autoScrollHotspot_offsetTop, 58);
    ...
    protected void onMeasure(int widthSpec, int heightSpec) {
        ...
        if (mHotspotHeight > -1) {
            ...
            mHotspotBottomBoundEnd = getMeasuredHeight() - mHotspotOffsetBottom + mHotspotHeight;
    ...
    /**
     * We intercept the touch handling only to support fast scrolling when
     * initiated from the scroll bar. Otherwise, we fall back to the default
    ...
    private boolean handleTouchEvent(MotionEvent ev) {
        ...
        if (mDragSelectActive) {
            itemPosition = getItemPosition(ev);
            switch (action) {
                ...
                case MotionEvent.ACTION_MOVE:
                    ...
                    if (mHotspotHeight > -1) {
                        if (ev.getY() >= mHotspotTopBoundStart && ev.getY() <= mHotspotTopBoundEnd) {
                            ...
                            final float simulatedY = ev.getY() - mHotspotTopBoundStart;
                            mAutoScrollVelocity = (int) (simulatedFactor - simulatedY) / 8;

                            LOG("Auto scroll velocity = %d", mAutoScrollVelocity);
                        } else if (ev.getY() >= mHotspotBottomBoundStart - 180 && ev.getY() <= mHotspotBottomBoundEnd - 180) {
                            mInTopHotspot = false;
                            if (!mInBottomHotspot) {
                            ...
                            final float simulatedY = ev.getY() + mHotspotBottomBoundEnd;
                            final float simulatedFactor = mHotspotBottomBoundStart + mHotspotBottomBoundEnd;
                            mAutoScrollVelocity = Math.abs((int) (simulatedY - simulatedFactor) / 8);
