/*
 * Copyright (C) 2010 The Android Open Source Project
 *
 * Licensed under the Apache License, Version 2.0 (the "License");
 * you may not use this file except in compliance with the License.
 * You may obtain a copy of the License at
 *
 *      http://www.apache.org/licenses/LICENSE-2.0
 *
 * Unless required by applicable law or agreed to in writing, software
 * distributed under the License is distributed on an "AS IS" BASIS,
 * WITHOUT WARRANTIES OR CONDITIONS OF ANY KIND, either express or implied.
 * See the License for the specific language governing permissions and
 * limitations under the License.
 */
/* ----------|----------------------|----------------------|----------------- */
/* 04/02/2015|ye.chen               |PR922192              |[Monitor][Gallery]Display 3 same folder in location
/* ----------|----------------------|----------------------|----------------- */
/* 04/21/2015|chengbin.du           |PR970033              |[Enduser Feedback][Monitor][SDCard]NO images available after set SD card as default storage */
/* ----------|----------------------|----------------------|----------------- */
/* 05/05/2015|chengbin.du           |PR993696              |[Gallery_02][Video Category Layout] */
/* ----------|----------------------|----------------------|----------------- */
/* 25/05/2015 |    jialiang.ren     |      PR-1007281         |[Android][Gallery_v5.1.13.1.0204.0]The video location */
/*                                                             will change after munting or trimming a video         */
/*------------|---------------------|-------------------------|------------------------------------------------------*/
/* 06/11/2015|    su.jiang          |  PR-857659           |[Android5.1][Gallery_v5.2.3.1.0310.0]The picture which display not the select one*/
/*-----------|----------------------|----------------------|---------------------------------------------------------------------------------*/
/* 06/01/2015|    su.jiang          |  PR-1274456          |[Camera]The number of photo displays error in gallery screen after Burst shoot*/
/*-----------|----------------------|----------------------|------------------------------------------------------------------------------*/
/* 2016/01/14|  caihong.gu-nb       |  PR-1201923          |    [GAPP][Android 6.0][Gallery][Force close][REG]Gallery display “Gallery has stopped”when click trim button on video*/
/* ----------|----------------------|----------------------|----------------- */
/* 01/14/2016| jian.pan1            |[ALM]Defect:791983    |can not delete a parallax photo
/* ----------|----------------------|----------------------|----------------- */

package com.tct.gallery3d.data;

import android.content.BroadcastReceiver;
import android.content.Context;
import android.content.Intent;
import android.content.IntentFilter;
import android.database.ContentObserver;
import android.net.Uri;
import android.os.Handler;

import com.tct.gallery3d.app.GalleryApp;
import com.tct.gallery3d.app.StitchingChangeListener;
import com.tct.gallery3d.common.Utils;
import com.tct.gallery3d.data.MediaObject.PanoramaSupportCallback;
import com.tct.gallery3d.data.MediaSet.ItemConsumer;
import com.tct.gallery3d.data.MediaSource.PathId;
import com.tct.gallery3d.picasasource.PicasaSource;
import com.tct.gallery3d.util.GalleryUtils;

import java.util.ArrayList;
import java.util.Comparator;
import java.util.HashMap;
import java.util.Iterator;
import java.util.LinkedHashMap;
import java.util.List;
import java.util.Map.Entry;
import java.util.WeakHashMap;

// DataManager manages all media sets and media items in the system.
//
// Each MediaSet and MediaItem has a unique 64 bits id. The most significant
// 32 bits represents its parent, and the least significant 32 bits represents
// the self id. For MediaSet the self id is is globally unique, but for
// MediaItem it's unique only relative to its parent.
//
// To make sure the id is the same when the MediaSet is re-created, a child key
// is provided to obtainSetId() to make sure the same self id will be used as
// when the parent and key are the same. A sequence of child keys is called a
// path. And it's used to identify a specific media set even if the process is
// killed and re-created, so child keys should be stable identifiers.

public class DataManager implements StitchingChangeListener {
    public static final int INCLUDE_IMAGE = 1;
    public static final int INCLUDE_VIDEO = 2;
    public static final int INCLUDE_ALL = INCLUDE_IMAGE | INCLUDE_VIDEO;
    public static final int INCLUDE_LOCAL_ONLY = 4;
    public static final int INCLUDE_LOCAL_IMAGE_ONLY =
            INCLUDE_LOCAL_ONLY | INCLUDE_IMAGE;
    public static final int INCLUDE_LOCAL_VIDEO_ONLY =
            INCLUDE_LOCAL_ONLY | INCLUDE_VIDEO;
    public static final int INCLUDE_LOCAL_ALL_ONLY =
            INCLUDE_LOCAL_ONLY | INCLUDE_IMAGE | INCLUDE_VIDEO;
    public static final int INCLUDE_LOCAL_CAMERA = 8;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-04-21,PR970033
    public static final int INCLUDE_LOCAL_ALL_VIDEO = 16;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696
    public static final int INCLUDE_LOCAL_MOMENTS = 32;
    public static final int INCLUDE_LOCAL_FACESHOW = 64;
    public static final int INCLUDE_LOCAL_FAVORITE = 128;
    public static final int INCLUDE_LOCAL_CAMERA_REVIEW = 256;
    public static final int INCLUDE_PARALLAX = 512;
    public static final int INCLUDE_SLOWMOTION = 1024; // MODIFIED by Yaoyu.Yang, 2016-07-28,BUG-2208330
    public static final int INCLUDE_LOCAL_PRIVATE = 2048;

    // Any one who would like to access data should require this lock
    // to prevent concurrency issue.
    public static final Object LOCK = new Object();

    public static DataManager from(Context context) {
        GalleryApp app = (GalleryApp) context.getApplicationContext();
        return app.getDataManager();
    }

    private static final String TAG = "DataManager";

    // This is the path for the media set seen by the user at top level.
    private static final String TOP_SET_PATH = "/combo/{/local/all,/picasa/all}";

    private static final String TOP_IMAGE_SET_PATH = "/combo/{/local/image,/picasa/image}";

    private static final String TOP_VIDEO_SET_PATH = "/combo/{/local/video,/picasa/video}"; // MODIFIED by Yaoyu.Yang, 2016-08-04,BUG-2208330

    private static final String TOP_LOCAL_SET_PATH = "/local/all";

    private static final String TOP_LOCAL_IMAGE_SET_PATH = "/local/image";

    private static final String TOP_LOCAL_VIDEO_SET_PATH = "/local/video";

    private static final String TOP_LOCAL_CAMERA_SET_PATH = "/local/camera";//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-04-21,PR970033

    private static final String TOP_LOCAL_ALL_VIDEOS_SET_PATH = "/local/all/video";//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696

    private static final String TOP_LOCAL_MOMENTS_SET_PATH = "/local/moments";

    private static final String TOP_LOCAL_FACESHOW_SET_PATH = "/local/faceshow"; // MODIFIED by Yaoyu.Yang, 2016-07-28,BUG-2208330

    private static final String TOP_LOCAL_FAVORITE_SET_PATH = "/local/favorite";

    private static final String TOP_LOCAL_CAMERAREVIEW_SET_PATH = "/local/camerareview";

    private static final String TOP_LOCAL_PARALLAX = "/local/parallax";

    private static final String TOP_LOCAL_SLOWMOTION = "/local/slowmotion"; // MODIFIED by Yaoyu.Yang, 2016-07-28,BUG-2208330

    private static final String TOP_LOCAL_PRIVATE_SET_PATH = "/local/private";

    public static final Comparator<MediaItem> sDateTakenComparator =
            new DateTakenComparator();

    private static WeakHashMap<ChangeNotifier, Object> sNotifiers =
            new WeakHashMap<ChangeNotifier, Object>();
    private static boolean sIsOperation = false;

    private static class DateTakenComparator implements Comparator<MediaItem> {
        @Override
        public int compare(MediaItem item1, MediaItem item2) {
            //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-05-25,PR1007281 begin
//            return -Utils.compare(item1.getDateInMs(), item2.getDateInMs());
            return -Utils.compare(item1.getDateModifiedInMs(), item2.getDateModifiedInMs());
            //[BUGFIX]-Add by TCTNJ,jialiang.ren, 2015-05-25,PR1007281 end
        }
    }

    public boolean isFaceShowAction = false;//[BUGFIX]-Add by TCTNJ,su.jiang, 2015-11-06,PR857659
    public boolean isNotFromThirdPart = true;//[BUGFIX]-Modify by TCTNJ,su.jiang, 2016-01-06,PR1274456
    private final Handler mDefaultMainHandler;

    private GalleryApp mApplication;
    private int mActiveCount = 0;

    private HashMap<Uri, NotifyBroker> mNotifierMap =
            new HashMap<Uri, NotifyBroker>();

    private HashMap<String, MediaSource> mSourceMap =
            new LinkedHashMap<String, MediaSource>();

    public void notifyPrivateMode() {
        Iterator<NotifyBroker> iterator = mNotifierMap.values().iterator();
        while (iterator.hasNext()) {
            NotifyBroker broker = iterator.next();
            broker.onChange(true);
        }
    }

    // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:791983 begin
    public void updateFyuseView() {
        if (parallaxObservers != null) {
            for (int i = 0; i < parallaxObservers.size(); i++) {
                parallaxObservers.get(i).onParallaxChanged();
            }
        }
    }
    // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:791983 end

    private List<ParallaxSourceListener> parallaxObservers = new ArrayList<ParallaxSourceListener>();
    public static interface ParallaxSourceListener {
        public void onParallaxChanged();
    }

    public void registerParallaxListener(ParallaxSourceListener listener) {
        if(!parallaxObservers.contains(listener)) {
            parallaxObservers.add(listener);
        }
    }

    public void unregisterParallaxListener(ParallaxSourceListener listener) {
        if(parallaxObservers.contains(listener)) {
            parallaxObservers.remove(listener);
        }
    }

    public DataManager(GalleryApp application) {
        mApplication = application;
        mDefaultMainHandler = new Handler(application.getMainLooper());
        IntentFilter filter = new IntentFilter();
        filter.addAction(GalleryUtils.INTENT_OPERATION_START);
        filter.addAction(GalleryUtils.INTENT_OPERATION_FINISH);
        mApplication.getAndroidContext().registerReceiver(mReceiver, filter);
    }

    private HashMap<Uri, String> mUriInfoMap = null;
    public boolean isCameraReviewInit = false;

    public boolean getCameraReviewInit() {
        return isCameraReviewInit;
    }

    public void setCameraReviewInit(boolean flag) {
        isCameraReviewInit = flag;
    }

    public HashMap<Uri, String> getUriInfoMap() {
        return mUriInfoMap;
    }

    public void setUriInfoMap(HashMap<Uri, String> mUriInfoMap) {
        isCameraReviewInit  =true;
        this.mUriInfoMap = mUriInfoMap;
    }

    public synchronized void initializeSourceMap() {
        if (!mSourceMap.isEmpty()) return;

        // the order matters, the UriSource must come last
        addSource(new LocalSource(mApplication));
        addSource(new PicasaSource(mApplication));
        addSource(new ComboSource(mApplication));
        addSource(new ClusterSource(mApplication));
        addSource(new FilterSource(mApplication));
        addSource(new SecureSource(mApplication));
        addSource(new UriSource(mApplication));

        if (mActiveCount > 0) {
            for (MediaSource source : mSourceMap.values()) {
                source.resume();
            }
        }
    }

    public String getTopSetPath(int typeBits) {

        switch (typeBits) {
            case INCLUDE_IMAGE: return TOP_IMAGE_SET_PATH;
            case INCLUDE_VIDEO: return TOP_VIDEO_SET_PATH;
            case INCLUDE_ALL: return TOP_SET_PATH;
            case INCLUDE_LOCAL_IMAGE_ONLY: return TOP_LOCAL_IMAGE_SET_PATH;
            case INCLUDE_LOCAL_VIDEO_ONLY: return TOP_LOCAL_VIDEO_SET_PATH;
            case INCLUDE_LOCAL_ALL_ONLY: return TOP_LOCAL_SET_PATH;
            case INCLUDE_LOCAL_CAMERA: return TOP_LOCAL_CAMERA_SET_PATH;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-04-21,PR970033
            case INCLUDE_LOCAL_ALL_VIDEO: return TOP_LOCAL_ALL_VIDEOS_SET_PATH;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696
            case INCLUDE_LOCAL_MOMENTS: return TOP_LOCAL_MOMENTS_SET_PATH;
            case INCLUDE_LOCAL_FACESHOW: return TOP_LOCAL_FACESHOW_SET_PATH;
            case INCLUDE_LOCAL_FAVORITE: return TOP_LOCAL_FAVORITE_SET_PATH;
            case INCLUDE_LOCAL_PRIVATE: return TOP_LOCAL_PRIVATE_SET_PATH;
            case INCLUDE_LOCAL_CAMERA_REVIEW: return TOP_LOCAL_CAMERAREVIEW_SET_PATH;
            case INCLUDE_PARALLAX: return TOP_LOCAL_PARALLAX;
            case INCLUDE_SLOWMOTION: return TOP_LOCAL_SLOWMOTION; // MODIFIED by Yaoyu.Yang, 2016-07-28,BUG-2208330
            default: throw new IllegalArgumentException();
        }
    }

    // open for debug
    void addSource(MediaSource source) {
        if (source == null) return;
        mSourceMap.put(source.getPrefix(), source);
    }

    // A common usage of this method is:
    // synchronized (DataManager.LOCK) {
    //     MediaObject object = peekMediaObject(path);
    //     if (object == null) {
    //         object = createMediaObject(...);
    //     }
    // }
    public MediaObject peekMediaObject(Path path) {
        return path.getObject();
    }

    public MediaObject getMediaObject(Path path) {
        synchronized (LOCK) {
            MediaObject obj = path.getObject();
         // [BUGFIX]-Add-BEGIN by TCTNJ.ye.chen,04/02/2015,PR922192
            if(obj != null){
                //[BUGFIX]-Add by TCTNJ,caihong.gu-nb, 2015/12/26,PR1201923 begin
                if(obj.getClass() != ClusterAlbumSet.class){//[Defect]-Modified by Yuanxi.Jiang,01/11/2016,defect1392871
                //[BUGFIX]-Add by TCTNJ,caihong.gu-nb, 2015/12/26,PR1201923 end
                    return obj;
                }else{
                    path.setObject();
                }
            }
         // [BUGFIX]-Add-BEGIN by TCTNJ.ye.chen,04/02/2015,PR922192
            MediaSource source = mSourceMap.get(path.getPrefix());
            if (source == null) {
                Log.w(TAG, "cannot find media source for path: " + path);
                return null;
            }

            try {
                MediaObject object = source.createMediaObject(path);
                if (object == null) {
                    Log.w(TAG, "cannot create media object: " + path);
                }
                return object;
            } catch (Throwable t) {
                Log.w(TAG, "exception in creating media object: " + path, t);
                return null;
            }
        }
    }

    public MediaObject getMediaObject(String s) {
        return getMediaObject(Path.fromString(s));
    }

    public MediaSet getMediaSet(Path path) {
        return (MediaSet) getMediaObject(path);
    }

    public MediaSet getMediaSet(String s) {
        return (MediaSet) getMediaObject(s);
    }

    public MediaSet[] getMediaSetsFromString(String segment) {
        String[] seq = Path.splitSequence(segment);
        int n = seq.length;
        MediaSet[] sets = new MediaSet[n];
        for (int i = 0; i < n; i++) {
            sets[i] = getMediaSet(seq[i]);
        }
        return sets;
    }

    // Maps a list of Paths to MediaItems, and invoke consumer.consume()
    // for each MediaItem (may not be in the same order as the input list).
    // An index number is also passed to consumer.consume() to identify
    // the original position in the input list of the corresponding Path (plus
    // startIndex).
    public void mapMediaItems(ArrayList<Path> list, ItemConsumer consumer,
            int startIndex) {
        HashMap<String, ArrayList<PathId>> map =
                new HashMap<String, ArrayList<PathId>>();

        // Group the path by the prefix.
        int n = list.size();
        for (int i = 0; i < n; i++) {
            Path path = list.get(i);
            String prefix = path.getPrefix();
            ArrayList<PathId> group = map.get(prefix);
            if (group == null) {
                group = new ArrayList<PathId>();
                map.put(prefix, group);
            }
            group.add(new PathId(path, i + startIndex));
        }

        // For each group, ask the corresponding media source to map it.
        for (Entry<String, ArrayList<PathId>> entry : map.entrySet()) {
            String prefix = entry.getKey();
            MediaSource source = mSourceMap.get(prefix);
            source.mapMediaItems(entry.getValue(), consumer);
        }
    }

    // The following methods forward the request to the proper object.
    public int getSupportedOperations(Path path) {
        return getMediaObject(path).getSupportedOperations();
    }

    public void getPanoramaSupport(Path path, PanoramaSupportCallback callback) {
        getMediaObject(path).getPanoramaSupport(callback);
    }

    public void delete(Path path) {
        getMediaObject(path).delete();
    }

    /* MODIFIED-BEGIN by Yaoyu.Yang, 2016-08-17,BUG-2208330*/
    private String mObjectPath;

    public void setObjectPath(String objectPath) {
        mObjectPath = objectPath;
    }

    public String move(Path path) {
        MediaObject mediaObject = getMediaObject(path);
        mediaObject.move(mObjectPath);
        return mediaObject.getGoalFile();
    }

    public String copy(Path path) {
        MediaObject mediaObject = getMediaObject(path);
        mediaObject.copy(mObjectPath);
        return mediaObject.getGoalFile();
    }
    /* MODIFIED-END by Yaoyu.Yang,BUG-2208330*/

    public void rotate(Path path, int degrees) {
        getMediaObject(path).rotate(degrees);
    }

    public Uri getContentUri(Path path) {
        return getMediaObject(path).getContentUri();
    }

    public int getMediaType(Path path) {
        return getMediaObject(path).getMediaType();
    }

    public Path findPathByUri(Uri uri, String type) {
        if (uri == null) return null;
        for (MediaSource source : mSourceMap.values()) {
            Path path = source.findPathByUri(uri, type);
            if (path != null) return path;
        }
        return null;
    }

    public Path getDefaultSetOf(Path item) {
        //[BUGFIX]-Add-BEGIN by TSNJ.zhe.xu, 2015.12.15, alm-1003393
        MediaSource source = null;
        if (item != null) {
            source = mSourceMap.get(item.getPrefix());
        }
        //[BUGFIX]-Add-END by TSNJ.zhe.xu, 2015.12.15, alm-1003393
        return source == null ? null : source.getDefaultSetOf(item);
    }

    // Returns number of bytes used by cached pictures currently downloaded.
    public long getTotalUsedCacheSize() {
        long sum = 0;
        for (MediaSource source : mSourceMap.values()) {
            sum += source.getTotalUsedCacheSize();
        }
        return sum;
    }

    // Returns number of bytes used by cached pictures if all pending
    // downloads and removals are completed.
    public long getTotalTargetCacheSize() {
        long sum = 0;
        for (MediaSource source : mSourceMap.values()) {
            sum += source.getTotalTargetCacheSize();
        }
        return sum;
    }

    public void registerChangeNotifier(Uri uri, ChangeNotifier notifier) {
        NotifyBroker broker = null;
        synchronized (mNotifierMap) {
            broker = mNotifierMap.get(uri);
            if (broker == null) {
                broker = new NotifyBroker(mDefaultMainHandler);
                mApplication.getContentResolver()
                        .registerContentObserver(uri, true, broker);
                mNotifierMap.put(uri, broker);
            }
        }
        broker.registerNotifier(notifier);
    }

    public void resume() {
        if (++mActiveCount == 1) {
            for (MediaSource source : mSourceMap.values()) {
                source.resume();
            }
        }
    }

    public void pause() {
        if (--mActiveCount == 0) {
            for (MediaSource source : mSourceMap.values()) {
                source.pause();
            }
        }
    }

    /**
     * add to know the operation progress
     */
    private BroadcastReceiver mReceiver = new BroadcastReceiver() {

        @Override
        public void onReceive(Context context, Intent intent) {
            String action = intent.getAction();
            Log.d(TAG, "action-----------" + action);
            if (GalleryUtils.INTENT_OPERATION_START.equals(action)) {
                sIsOperation = true;
            } else if (GalleryUtils.INTENT_OPERATION_FINISH.equals(action)) {
                sIsOperation = false;
                for (ChangeNotifier notifier : sNotifiers.keySet()) {
                    notifier.onChange(false);
                }
            }
        }
    };

    /**
     * add to unregister the receiver
     */
    public void release() {
        try {
            mApplication.getAndroidContext().unregisterReceiver(mReceiver);
        } catch (Exception e) {
            Log.d("TAG", e.getMessage());
        }
    }

    private static class NotifyBroker extends ContentObserver {

        public NotifyBroker(Handler handler) {
            super(handler);
            sNotifiers.clear();
        }

        public synchronized void registerNotifier(ChangeNotifier notifier) {
            sNotifiers.put(notifier, null);
        }

        @Override
        public synchronized void onChange(boolean selfChange) {
            if(!sIsOperation) {
                for (ChangeNotifier notifier : sNotifiers.keySet()) {
                    notifier.onChange(selfChange);
                }
            }
        }
    }

    @Override
    public void onStitchingQueued(Uri uri) {
        // Do nothing.
    }

    @Override
    public void onStitchingResult(Uri uri) {
        Path path = findPathByUri(uri, null);
        if (path != null) {
            MediaObject mediaObject = getMediaObject(path);
            if (mediaObject != null) {
                mediaObject.clearCachedPanoramaSupport();
            }
        }
    }

    @Override
    public void onStitchingProgress(Uri uri, int progress) {
        // Do nothing.
    }
}
