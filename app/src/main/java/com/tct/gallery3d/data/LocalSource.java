 ...
 */
/* ----------|----------------------|---------------------|-------------------*/
/* 13/04/2015|qiang.ding1           |PR975496             |[SMC][Crash]com.tct.gallery3d crashed*/
/* ----------|----------------------|---------------------|-------------------*/
...
class LocalSource extends MediaSource {
    ...
    private static final int LOCAL_CAMERA_ALBUM = 8;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-04-21,PR970033
    private static final int LOCAL_ALL_VIDEOS_ALBUM = 9;//[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696
    private static final int LOCAL_MOMENTS_ALBUM = 10;
    private static final int LOCAL_FACESHOW_ALBUM = 11;
    private static final int LOCAL_FAVORITE_ALBUM = 12;
    private static final int LOCAL_CAMERA_REVIEW_ALBUM = 13;
    private static final int LOCAL_SLOWMOTION_ALBUM = 14; // MODIFIED by Yaoyu.Yang, 2016-07-28,BUG-2208330
    private static final int LOCAL_SELECT_ALBUMSET = 15;
    private static final int LOCAL_PRIVATE_ALBUM = 16;
    private static final String TAG = "LocalSource";

    private static final int LOCAL_COLLAPSED_ALBUMSET = 17;
    ...
    public LocalSource(GalleryApp context) {
        ...
        mMatcher.add(LOCAL_SET_PATH, LOCAL_ALL_ALBUMSET);
        mMatcher.add(LOCAL_SELECT_PATH, LOCAL_SELECT_ALBUMSET);
        mMatcher.add(LOCAL_IMAGE_SET_PATH, LOCAL_IMAGE_ALBUMSET);
        mMatcher.add(LOCAL_VIDEO_SET_PATH, LOCAL_VIDEO_ALBUMSET);

        mMatcher.add(LOCAL_ALBUM_PATH, LOCAL_ALL_ALBUM);
        mMatcher.add(LOCAL_IMAGE_ALBUM_PATH, LOCAL_IMAGE_ALBUM);
        mMatcher.add(LOCAL_VIDEO_ALBUM_PATH, LOCAL_VIDEO_ALBUM);
        mMatcher.add(LOCAL_IMAGE_ITEM_PATH, LOCAL_IMAGE_ITEM);
        mMatcher.add(LOCAL_VIDEO_ITEM_PATH, LOCAL_VIDEO_ITEM);
        mMatcher.add(LOCAL_ALL_VIDEOS_SET_PATH, LOCAL_ALL_VIDEOS_ALBUM);

        mMatcher.add(LOCAL_CAMERA_SET_PATH, LOCAL_CAMERA_ALBUM);
        mMatcher.add(LOCAL_MOMENTS_SET_PATH, LOCAL_MOMENTS_ALBUM);
        mMatcher.add(LOCAL_FACESHOW_SET_PATH, LOCAL_FACESHOW_ALBUM);
        mMatcher.add(LOCAL_FAVORITE_SET_PATH, LOCAL_FAVORITE_ALBUM);
        mMatcher.add(LOCAL_SLOWMOTION_SET_PATH, LOCAL_SLOWMOTION_ALBUM);
        mMatcher.add(LOCAL_PRIVATE_SET_PATH, LOCAL_PRIVATE_ALBUM);
        /* MODIFIED-END by Yaoyu.Yang,BUG-2208330*/
        mMatcher.add("/local/camerareview", LOCAL_CAMERA_REVIEW_ALBUM);
        ...
        mMatcher.add(LOCAL_COLLAPSED_PATH,LOCAL_COLLAPSED_ALBUMSET);
    }
    ...
    public MediaObject createMediaObject(Path path) {
        ...
        switch (mMatcher.match(path)) {
            ...
            case LOCAL_ALL_ALBUM: {
                ...
                /* MODIFIED-BEGIN by Yaoyu.Yang, 2016-08-04,BUG-2208330*/
//                MediaSet faceShowSet = (MediaSet) dataManager.getMediaObject(
//                        LocalAlbumSet.PATH_FACESHOW.getChild(bucketId));
//                MediaSet slowMotionSet = (MediaSet) dataManager.getMediaObject(
//                        LocalAlbumSet.PATH_SLOWMOTION.getChild(bucketId));
            ...
            case LOCAL_CAMERA_ALBUM: {
                DataManager dataManager = app.getDataManager();
                MediaSet internalCameraImageSet = (MediaSet) dataManager.getMediaObject(
                    LocalAlbumSet.PATH_IMAGE.getChild(MediaSetUtils.CAMERA_BUCKET_ID));
                MediaSet internalCameraVideoSet = (MediaSet) dataManager.getMediaObject(
                    LocalAlbumSet.PATH_VIDEO.getChild(MediaSetUtils.CAMERA_BUCKET_ID));
                MediaSet sdcardCameraImageSet = (MediaSet) dataManager.getMediaObject(
                    LocalAlbumSet.PATH_IMAGE.getChild(MediaSetUtils.SDCARD_CAMERA_BUCKET_ID));
                MediaSet sdcardCameraVideoSet = (MediaSet) dataManager.getMediaObject(
                    LocalAlbumSet.PATH_VIDEO.getChild(MediaSetUtils.SDCARD_CAMERA_BUCKET_ID));
                Comparator<MediaItem> comp = DataManager.sDateTakenComparator;
                MediaSet[] mediasets = { internalCameraImageSet, internalCameraVideoSet, sdcardCameraImageSet, sdcardCameraVideoSet};
                return new LocalMergeAlbum(path, comp, mediasets, MediaSetUtils.CAMERA_BUCKET_ID);
            }
            //[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-04-21,PR970033 end
            //[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696 begin
            case LOCAL_ALL_VIDEOS_ALBUM:
                return new LocalVideoAlbum(path, mApplication);
            //[BUGFIX]-Add by TCTNJ,chengbin.du, 2015-05-05,PR993696 end
            case LOCAL_MOMENTS_ALBUM:
                return new MomentsNewAlbum(path, mApplication);
            ...
            case LOCAL_FACESHOW_ALBUM:
                return new FaceShowAlbum(path, mApplication);
            case LOCAL_CAMERA_REVIEW_ALBUM:
                return new CameraReviewAlbum(path, mApplication);
            // MODIFIED-BEGIN by Yaoyu.Yang, 2016-07-28, BUG-2208330
            case LOCAL_SLOWMOTION_ALBUM:
                return new SlowMotionAlbum(path, mApplication);
                // MODIFIED-END by Yaoyu.Yang, BUG-2208330
            case LOCAL_SELECT_ALBUMSET:
                return new SelectAlbumSet(path, mApplication);
            case LOCAL_PRIVATE_ALBUM:
                return new PrivateAlbum(path, mApplication);
            case LOCAL_COLLAPSED_ALBUMSET:
                return new CollapseAlbumSet(path,mApplication);
            default:
    ...
    public void mapMediaItems(ArrayList<PathId> list, ItemConsumer consumer) {
        ...
        ArrayList<PathId> imageFyuseList = new ArrayList<PathId>();
        ...
        for (int i = 0; i < n; i++) {
            ...
            if (parent == LocalImage.ITEM_PATH) {
                ...
                videoList.add(pid);
                // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 begin
            } else if (parent == ParallaxItem.ITEM_PATH) {
                imageFyuseList.add(pid);
            }
            // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 end
        }
        ...
        // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 begin
        processFyuseMapMediaItems(imageFyuseList, consumer);
        // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 end
    ...
    // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 begin
    private void processFyuseMapMediaItems(ArrayList<PathId> list,
        ...
        MediaItem[] items = MomentsAlbum.getFyuseItemsByPaths(dataManager,
                paths);

        for (int i = 0; i < n; i++) {
            PathId pid = list.get(i);
            consumer.consume(pid.id, items[i]);
        }
    }
    // [ALM][BUGFIX]-Add by TCTNJ,jian.pan1, 2015-01-14,Defect:1270036 end
    ...
    @Override
    public void pause() {
    //[BUGFIX]-Add by TCTNJ,qiang.ding1, 2015-04-13,PR975496  begin
       if(mClient != null){
           mClient.release();
           mClient = null;
       }
     //[BUGFIX]-Add by TCTNJ,qiang.ding1, 2015-04-13,PR975496  end
    }
}
