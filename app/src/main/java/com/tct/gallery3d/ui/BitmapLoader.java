...
public abstract class BitmapLoader implements FutureListener<Bitmap> {
    ...
    public synchronized void recycleExceptBitmap() {
        mState = STATE_RECYCLED;
        if (mBitmap != null) {
            mBitmap = null;
        }
    ...
    public synchronized Bitmap getBitmap(int rotation) {
        if(rotation != 0) {
            Bitmap tmp = BitmapUtils.rotateBitmap(mBitmap, rotation, false);
            if(tmp != null) {
                mBitmap.recycle();
                mBitmap = tmp;
            }
