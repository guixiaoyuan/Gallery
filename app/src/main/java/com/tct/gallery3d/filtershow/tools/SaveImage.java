...
/* ----------|----------------------|----------------------|---------------------------------------- */
/* 16/03/2015|dongliang.feng        |PR950280              |[Android5.0][Callery_v5.1.9.1.0104.0][Hoperun] */
/*           |                      |                      |Picture size will be small after use filter */
/* ----------|----------------------|----------------------|---------------------------------------- */
...
/* ----------|----------------------|----------------------|----------------- */
/* 05/16/2015| jian.pan1            | PR1002886            |[5.0][Gallery] cropped image detail still show original uncropped resolution
/* ----------|----------------------|----------------------|----------------- */
/* 01/07/2015|dongliang.feng        |PR1033187             |[SW][Gallery][FC]Gallery will fc when export picture after delete it */
/* ----------|----------------------|----------------------|---------------------------------------- */
/* 03/04/2016| jian.pan1            |[ALM]Defect:1660727   |[onetouch feedback]edit screentshot in moments layout display error.
/* ----------|----------------------|----------------------|----------------- */
/* 03/05/2016|    su.jiang          |  PR-1533452          |[Gallery]After edit and save as copy the self picture it can't show the edited self picture.*/
/*-----------|----------------------|----------------------|--------------------------------------------------------------------------------------------*/
...
import com.tct.gallery3d.util.XmpUtilHelper;
...
public class SaveImage {
    ...
    private static File getCopyToFile(String filename, String parentFile) {
        File toFile = new File(parentFile + "/" + filename);
        return toFile;
    }

    public static String copyFile(File fromFile, String toPath, MediaObject object) {
        if (!fromFile.exists()) {
            return null;
        }
        String fileName = fromFile.getName();
        if (getCopyToFile(fileName, toPath).exists()) {
            int index = 0;
            int firstDotPos = fileName.indexOf(".");
            int firstBracket = fileName.indexOf("(");
            if (firstDotPos != -1) {
                final String filenameNoExt = (firstBracket == -1) ? fileName.substring(0, firstDotPos)
                        : fileName.substring(0, firstBracket);
                final String nameExt = fileName.substring(firstDotPos, fileName.length());
                while (getCopyToFile(filenameNoExt + "(" + index + ")" + nameExt, toPath).exists()) {
                    index++;
                }
                fileName = filenameNoExt + "(" + index + ")" + nameExt;
            }
        }
        String toFilePath;
        if (toPath.endsWith(File.separator)) {
            ...
            toFilePath = toPath + File.separator + fileName;
        }
        ...
        try {
            ...
            fosto = new FileOutputStream(toFilePath);
            byte buffer[] = new byte[1024];
            while ((byteread = fosfrom.read(buffer)) > 0) {
                if (object.getCancelStatus()) {
                    break;
                }
    ...
    public Uri processAndSaveImage(ImagePreset preset, boolean flatten,
        ...
        //[BUGFIX]-Add by TCTNJ,su.jiang, 2016-03-05,PR1533452 begin
        isBurstShotPicture = ExifInfoFilter.getInstance(mContext).queryType(
                newSourceUri.getLastPathSegment()) == ExifInfoFilter.BURSTSHOTS;
        //[BUGFIX]-Add by TCTNJ,su.jiang, 2016-03-05,PR1533452 end
        if (!flatten) {
        ...
        if (mPreviewImage != null) {
            if (flatten) {
                ...
                //[BUGFIX]-Add by TCTNJ,su.jiang, 2016-03-05,PR1533452 begin
                if (!isBurstShotPicture) {
                    String userComment = exif.getUserComment();
                    if (userComment != null) {
                        isBurstShotPicture = userComment.contains("burst_shot");
                    }
                }
                if (isBurstShotPicture) {
...
//                        }
    ...
    public static void querySourceFromContentResolver( // MODIFIED by Yaoyu.Yang, 2016-08-17,BUG-2208330
  ...
  //[BUGFIX]-Modify by TCTNJ, ye.chen, 2015-03-17, PR949845 begin
    ...
    public static Uri linkNewFileToUri(Context context, Uri sourceUri,
        ...
        if (fileUri || oldSelectedFile == null || !deleteOriginal || isBurstShotPicture) {
        ...
        if (isBurstShotPicture) {
            context.getContentResolver().delete(sourceUri, "_id=?",new String[]{sourceUri.getLastPathSegment()});
            ExifInfoFilter.getInstance(context).removeImageTypeId(sourceUri.getLastPathSegment());
        }
