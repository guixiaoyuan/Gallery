...
package com.tct.gallery3d.exif;
...
class ExifData {
    ...
    protected String getUserComment() {
        //[FEATURE]-Modify by TCTNJ, dongliang.feng, 2015-01-09, FR898833 begin
        IfdData ifdData = mIfdDatas[IfdId.TYPE_IFD_EXIF];
        if (ifdData == null) {
            return null;
        }
        ...
        System.arraycopy(buf, 0, code, 0, 3);

        try {
            if (Arrays.equals(code, USER_COMMENT_ASCII)) {
                return new String(buf, 3, buf.length - 3, "US-ASCII");
            } else if (Arrays.equals(code, USER_COMMENT_JIS)) {
                return new String(buf, 3, buf.length - 3, "EUC-JP");
            } else if (Arrays.equals(code, USER_COMMENT_UNICODE)) {
                return new String(buf, 3, buf.length - 3, "UTF-16");
            } else {
                return new String(buf, 0, buf.length, "US-ASCII");
            }
        } catch (UnsupportedEncodingException e) {
            Log.w(TAG, "Failed to decode the user comment");
            return null;
        }
        //[FEATURE]-Modify by TCTNJ, dongliang.feng, 2015-01-09, FR898833 end
    }
